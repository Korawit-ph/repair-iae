...
export default function RepairList({ user }) {
  ...
  const filters = ['all', 'pending', 'in_progress', 'waiting_review', 'returned', 'completed'];

  return (
    <div style={{ padding: '1.5rem' }}>
      <h2 style={{ margin: '0 0 1.5rem', fontSize: 18, fontWeight: 700 }}>{tx.repairList}</h2>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          <thead>
            <tr style={{ background: '#f8fafc' }}>
              {[lang === 'th' ? 'เลขที่' : 'No.', tx.reporter, tx.device, tx.location, lang === 'th' ? 'ผู้รับผิดชอบ' : 'Assigned', lang === 'th' ? 'วันที่' : 'Date', lang === 'th' ? 'สถานะ' : 'Status', ''].map(h => (
          ...
          <tbody>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={8} style={{ padding: '2rem', textAlign: 'center', color: '#94a3b8' }}>ไม่มีรายการ</td></tr>
            )}
          </tbody>
        </table>
      </div>
